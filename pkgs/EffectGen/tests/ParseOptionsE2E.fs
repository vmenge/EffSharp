  ...
  let tests =
    testSequenced <| testList "ParseOptionsE2E" [
      testTask "generator honors project-defined conditional compilation when discovering [<Effect>] interfaces" {
        ...
        Expect.equal result.ExitCode 0 $"fixture {fixtureName} should build successfully when EffectGen uses the consumer parse options. Output:{System.Environment.NewLine}{result.Output}"
